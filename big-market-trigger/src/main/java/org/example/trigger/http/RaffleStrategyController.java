...
public class RaffleStrategyController implements IRaffleStrategyService {
    ...
    @Override
    public Response<Boolean> strategyAssembly(@RequestParam Long strategyId) {
        try {
...
//            log.info("\n测试打印： userId: {}, activityId: {}, 用户当日参与抽奖次数: {}\n", request.getUserId(), request.getActivityId(), accountDailyLottery);
     ...
     *
     * @param: request
     * @return: Response<List<RaffleStrategyRuleWeightResponseDTO>>
     **/
    ...
    public Response<List<RaffleStrategyRuleWeightResponseDTO>> queryRaffleStrategyRuleWeight(@RequestBody RaffleStrategyRuleWeightRequestDTO request) {
        try {
            log.info("查询抽奖策略权重 规则配置 开始 userId:{} activityId:{}", request.getUserId(), request.getActivityId());
            ...
            // 2. 查询用户抽奖总次数
            Integer userActivityAccountTotalUse = raffleActivityAccountQuotaService.queryAccountTotalLottery(request.getActivityId(), request.getUserId());
            ...
            for (RuleWeightVO ruleWeightVO : ruleWeightVOList) {
                ...
                responseDTO.setStrategyAwardList(strategyAwards);
                responseDTO.setUserActivityAccountTotalUse(userActivityAccountTotalUse);
            ...
            Response<List<RaffleStrategyRuleWeightResponseDTO>> response = Response.<List<RaffleStrategyRuleWeightResponseDTO>>builder()
                    ...
                    .build();
            log.info("查询抽奖策略完成 规则配置 开始 userId:{} activityId:{}", request.getUserId(), request.getActivityId());
            return response;
        } catch (Exception e) {
            log.error("查询抽奖策略完成 规则配置 失败 userId:{} activityId:{}", request.getUserId(), request.getActivityId(), e);
